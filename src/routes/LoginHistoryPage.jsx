...
export default function LoginHistoryPage() {
  const { loginHistory, clearHistory } = useLoginHistory();
  ...
  const stats = useMemo(() => {
    const visitors = loginHistory.filter((e) => e.userType === 'visitor').length;
    const admins = loginHistory.filter((e) => e.userType === 'admin').length;
    return {
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="bg-blue-50 border border-blue-300 rounded-lg p-4 mb-8 text-sm text-blue-800">
            ...
            <p>Login records are stored locally on your device with date and time information captured from your system clock. These timestamps are not synchronized with an external server.</p>
          </div>
                  ...
                  <div key={date} className="bg-white rounded-2xl shadow-md border border-gray-200 overflow-hidden">
                    <div className="bg-gradient-to-r from-red-50 to-blue-50 border-b border-gray-200 px-6 py-4">
                      <h3 className="font-bold text-slate-900 flex items-center gap-2">
                        ...
                        <span className="text-sm font-normal text-slate-600 ml-2">({groupedByDate[date].length} logins)</span>
                      </h3>
                    </div>
                          ...
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex-grow">
                              <p className="font-semibold text-slate-900">
                                {entry.userType === 'visitor' ? '👤' : '🔐'} {entry.userName}
                              </p>
